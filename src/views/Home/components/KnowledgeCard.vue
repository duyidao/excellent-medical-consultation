...
<template>
    <div class="knowledge-card van-hairline--bottom">
        <div class="head">
            <van-image round
                class="avatar"
                :src="item.creatorAvatar"></van-image>
            <div class="info">
            ...
            <van-button class="btn"
                size="small"
                round>{{ item.likeFlag === 1 ? '已关注' : '+ 关注' }}</van-button>
        </div>
        <div class="body">
            <h3 class="title van-ellipsis">{{ item.title }}</h3>
            <p class="tag">
                <span v-for="(tag, i) in item.topic.split(',')" :key="i"># {{ tag }}</span>
            </p>
            <p class="intro van-multi-ellipsis--l2"
                v-html="item.content">
            </p>
            <div class="imgs">
                <van-image v-for="(img, index) in item.coverUrl"
                    :key="index"
                    :src="img" />
            </div>
            <p class="logs">
